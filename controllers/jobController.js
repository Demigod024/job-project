...
exports.getJobListing = async (request, response) => {
  try {
    ...
    let queriedJobs = new QueryObj(Job.find(), request.query)
      .sort()
      .filter()
      .limit()
      .paginate();
...
exports.recommendAJob = async (req, res) => {
  try {
    ...
    let jobsRecommended = await Job.findOne({ title: user.skills, experience: user.experience })
